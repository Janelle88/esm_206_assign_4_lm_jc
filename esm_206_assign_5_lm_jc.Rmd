...
##FIX THIS
In comparing between the two types of sections, it shows that a higher proportion of Salamanders are found in pools in the Old Growth sections compared to clear cut. 
...
In Mack Creek in 2017 (n = `r sample_og_cc`), the mean weight for salamanders found in cascades was `r round(mean_c, 2)` $\pm$ `r round(sd_c, 2)`, in pools the mean weight of salamanders was `r round(mean_p, 2)` $\pm$ `r round(sd_p, 2)`, and for side-channels the mean weight of salamanders was `r round(mean_sc, 2)` $\pm$ `r round(sd_sc, 2)` (one-way ANOVA, F (`r salamander_aov_summary[2]`) = `r round(salamander_aov_summary[7], 2)`, p = `r round(salamander_aov_summary[9], 3)`).
Using Tukey’s HSD, the post hoc pairwise testing shows that the mean weights of salamanders in pools differ significantly from salamanders in side-channels (p = `r round(salamander_post_hoc$creek_area[12], 3)`). The significant difference in mean weight between salamanders is only seen between salamanders in pools and side-channels. Despite this, the effect size is relatively small (`r round(cd_sc_p$estimate, 2)`).  Between salamanders in pools and cascades, there is no observed significant difference in means (p = `r round(salamander_post_hoc$creek_area[10], 3)`). Similarly, there is no observed significant difference for salamanders in side-channels and in cascades: (p = `r round(salamander_post_hoc$creek_area[11], 3)`).
