...
![Aquatic Vertebrate Population Study in Mack Creek](map.jpg)
...
# Conducr ttests for salamander weights in old growth and clear cut areas.
...
sample_og_cc <- sum(salamander_count$total, na.rm = TRUE)
...
```{r, fig.cap="Figure 6: ... Data: Andrews Forest LTER"}
...
cd_sc_p <- effsize::cohen.d(sc_weight$weight, p_weight$weight, na.rm = TRUE)
...
For salamanders in Mack Creek (n = `r sample_og_cc`), for salamanders found in cascades, the mean weight of salamanders was `r round(mean_c, 2)` $\pm$ `r round(sd_c, 2)`, for salamanders found in pools the mean weight of salamanders is `r round(mean_p, 2)` $\pm$ `r round(sd_p, 2)`, and for side-channels the mean weight of salamanders was `r round(mean_sc, 2)` $\pm$ `r round(sd_sc, 2)` (one-way ANOVA F(`r salamander_aov_summary[1]`, `rsalamander_aov_summary[2]`)

#### Summary

- Graph A findings + table B findings
- Graph C findings
- Graph E findings
